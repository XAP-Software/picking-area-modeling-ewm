...
Внешний интерфейс отвечает за пользовательский ввод данных для параметров необходимых для запуска процесса моделирования. Внешний интерфейс включает в себя возможности просмотра предыдущих прогонов моделирования и изменения параметров подающихся на вход. После обработки пользовательского ввода, данные отправляются на серверную часть с помощью POST HTTP-запроса. 
...
Выбор внешнего интерфейса в качестве React.js обусловлен следующими причинами:
...
Серверная часть отвечает за получение данных необходимых для моделирования со стороны внешнего интерфейса, обрабатывает их и отправляет с помощью POST HTTP-запроса на сторону SAP S/4HANA System.

По окончанию завершения процесса моделирования, данные приходят в виде ответа на POST HTTP-запрос, сохраняются в базу данных MySQL и передаются во внешний интерфейс с помощью POST HTTP-запроса.

Выбор серверной части в качестве Python обусловлен следующими причинами:

- Высокая производительность в отношении обработки больших данных
- Высокий уровень поддержки сообщества
- Простота реализации

### База данных

База данных отвечает за хранение результатов предыдущих прогонов. 

Выбор MySQL в качестве базы данных обусловлен следующими причинами:

- Одна из самых легких реляционных баз данных
